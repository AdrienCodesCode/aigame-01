...
wide_eye::game::DogState run_forward(std::string_view scenario_name, int tick_count) {
    const auto scenario = wide_eye::game::find_gameplay_scenario(scenario_name);
...
int main() {
    ...
    const auto start = wide_eye::game::find_gameplay_scenario("paddock-start");
    const auto wall = wide_eye::game::find_gameplay_scenario("wall-contact");
    const auto closed_gate = wide_eye::game::find_gameplay_scenario("closed-gate");
    const auto open_gate = wide_eye::game::find_gameplay_scenario("open-gate");
    if (!check(start.has_value() && wall.has_value() && closed_gate.has_value() &&
                   open_gate.has_value() &&
                   !wide_eye::game::find_gameplay_scenario("unknown").has_value(),
    ...
    CameraController orbit_camera{start->dog.initial_state};
    const CameraState orbit_initial = orbit_camera.state();
    const auto orbit_initial_pose = orbit_camera.pose(start->dog.initial_state);
    orbit_camera.fixed_update(start->dog.initial_state,
    ...
    if (!check(approximately(orbit_camera.state().gameplay_yaw, orbit_initial.gameplay_yaw + 0.3),
        ...
        !check(orbit_camera.pose(start->dog.initial_state).eye != orbit_initial_pose.eye,
               "mouse_orbits_stationary_dog") ||
        !check(start->dog.initial_state.heading_radians == 0.0,
               "mouse_orbit_does_not_change_dog_heading")) {
        return EXIT_FAILURE;
    }

    CameraController delta_at_sixty{start->dog.initial_state};
    CameraController delta_at_thirty{start->dog.initial_state};
    delta_at_sixty.fixed_update(start->dog.initial_state,
                                CameraControlInput{.look_right_delta = 20.0}, 1.0 / 60.0);
    delta_at_thirty.fixed_update(start->dog.initial_state,
                                 CameraControlInput{.look_right_delta = 20.0}, 1.0 / 30.0);
    ...
    DogState previous_interpolation = start->dog.initial_state;
    DogState current_interpolation = start->dog.initial_state;
    ...
    for (int tick = 0; tick < 120; ++tick) {
        ...
        first_contact_motor.fixed_update({.world_z = -1.0}, 1.0 / 60.0);
        if (previous_z > 16.0 + DogController::kRadius &&
            approximately(first_contact_motor.state().position.z, 16.0 + DogController::kRadius,
                          1.0e-12)) {
            observed_first_contact = true;
            if (!check(first_contact_motor.state().velocity.z == 0.0,
                       "first_contact_clears_blocked_velocity")) {
                return EXIT_FAILURE;
            }
            break;
        }
    }
    if (!check(observed_first_contact, "first_contact_observed")) {
        return EXIT_FAILURE;
    }
    ...
    if (!check(steering_dog.state().position.x > start->dog.initial_state.position.x &&
                   steering_dog.state().position.z < start->dog.initial_state.position.z,
